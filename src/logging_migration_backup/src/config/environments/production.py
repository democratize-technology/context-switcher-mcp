    ...
    def validate_environment_requirements(self) -> list[str]:
        ...
        for var in critical_env_vars:
            if not os.getenv(var):
                warnings.append(f"❌ CRITICAL: Required environment variable not set: {var}")
            else:
                # Validate secret key strength
                if var == "CONTEXT_SWITCHER_SECRET_KEY":
                    key = os.getenv(var)
                    if len(key) < 64:
                        warnings.append("⚠ Secret key should be at least 64 characters for production")
        ...
        if backend_warnings:
            warnings.append(f"❌ Backend credentials missing: {', '.join(backend_warnings)}")
        
        # Check security settings
        if os.getenv("CS_HOST") == "localhost":
            warnings.append("❌ CRITICAL: Server bound to localhost - not accessible externally")
        ...
        for dep in required_deps:
            try:
                ...
                warnings.append(f"❌ CRITICAL: Required dependency not installed: {dep}")
