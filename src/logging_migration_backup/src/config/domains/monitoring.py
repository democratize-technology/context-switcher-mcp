...
class MetricsConfig(BaseModel):
    ...
    enable_system_metrics: bool = Field(
        default=True,
        description="Enable system resource metrics collection"
    )
    
    enable_application_metrics: bool = Field(
        default=True,
        description="Enable application-level metrics"
    )
...
class ProfilingConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True,
        description="Enable LLM profiling"
    )
    ...
    track_tokens: bool = Field(
        default=True,
        description="Track token usage in API calls"
    )
    ...
    track_model_performance: bool = Field(
        default=True,
        description="Track model-specific performance metrics"
    )
...
class AlertingConfig(BaseModel):
    ...
    enable_performance_alerts: bool = Field(
        default=True,
        description="Enable performance threshold alerts"
    )
...
class MonitoringConfig(BaseSettings):
    ...
    enable_monitoring: bool = Field(
        ...
        env="CS_ENABLE_MONITORING"
    )
    
    monitoring_storage_path: Optional[str] = Field(
        default=None,
    ...
    def is_production_monitoring(self) -> bool:
        ...
        return (
            ...
            self.profiling.level in [ProfilingLevel.BASIC, ProfilingLevel.STANDARD] and
            self.alerting.enabled and
            self.metrics.retention_days >= 7
        )
    ...
    def should_profile_call(
        ...
        if latency_seconds and latency_seconds > self.profiling.latency_alert_threshold_s:
            if self.profiling.always_profile_slow_calls:
                return True
        
        if cost_usd and cost_usd > (self.profiling.cost_alert_threshold_usd / 1000):  # Per-call threshold
