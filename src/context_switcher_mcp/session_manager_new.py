...
class SimpleSessionManager:
    ...
    async def create_session(
        ...
        async with self._global_lock:
            ...
            if len(self._sessions) >= self.max_sessions:
                ...
                if len(self._sessions) >= self.max_sessions:
                    raise SessionError(
                        f"Session capacity exceeded ({self.max_sessions})"
                    )

            # Create new session with built-in security
            session = Session(
                session_id=session_id, topic=topic, create_client_binding=True
            )
            ...
            if initial_perspectives:
                ...
                for perspective_name in initial_perspectives:
                    ...
                    if system_prompt:
                        ...
                        logger.warning(
                            f"Unknown perspective '{perspective_name}', skipping"
                        )
    ...
    async def _remove_session_internal(self, session_id: str) -> bool:
        ...
        logger.info(
            f"Removed session '{session_id}' (pool size: {len(self._sessions)})"
        )
        return True
        ...
        async with self._global_lock:
            ...
            for session_id, session in self._sessions.items():
                try:
                    ...
                    logger.warning(
                        f"Error getting info for session '{session_id}': {e}"
                    )
        ...
        async with self._global_lock:
            ...
            return {
                ...
                "capacity_used_percent": round(
                    (active_count / self.max_sessions) * 100, 1
                ),
                "session_ttl_hours": self.session_ttl_hours,
                "cleanup_interval_seconds": self.cleanup_interval.total_seconds(),
                "total_threads": total_threads,
                "total_analyses": total_analyses,
                "oldest_session_age_seconds": oldest_session_age.total_seconds()
                if oldest_session_age
                else None,
                "newest_session_age_seconds": newest_session_age.total_seconds()
                if newest_session_age
                else None,
    ...
    async def _background_cleanup_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                if cleanup_count > 0:
                    logger.debug(
                        f"Background cleanup removed {cleanup_count} expired sessions"
                    )
            except Exception as e:
        ...
        async with self._global_lock:
            ...
            for session_id in session_ids:
                try:
                    ...
                    logger.warning(
                        f"Error cleaning up session '{session_id}' during shutdown: {e}"
                    )
