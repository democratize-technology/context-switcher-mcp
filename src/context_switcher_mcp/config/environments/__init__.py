...
from ..core import ContextSwitcherConfig, ConfigurationError, create_config
...
def detect_environment() -> str:
    ...
    if env in ENVIRONMENT_CONFIGS:
        return "development" if env in ["dev", "development"] else \
               "staging" if env in ["stage", "staging"] else \
               "production"
    ...
    if (
        ...
        os.getenv("LOG_LEVEL", "").upper() == "DEBUG" or
        os.path.exists(".env") or  # Development environment file
        os.path.exists("pyproject.toml")  # Development project
    ):
        return "development"
