...
class StagingConfig(BaseEnvironmentConfig):
    ...
    def validate_environment_requirements(self) -> list[str]:
        ...
        if not os.getenv("AWS_PROFILE") and not os.getenv("AWS_ACCESS_KEY_ID"):
            warnings.append("⚠ AWS credentials not configured - Bedrock backend unavailable")
        ...
        if os.getenv("CS_HOST") == "localhost":
            warnings.append("⚠ Server bound to localhost - may not be accessible externally")
        ...
        for dep in optional_deps:
            try:
                ...
                warnings.append(f"⚠ {dep} not installed - some features may be unavailable")
