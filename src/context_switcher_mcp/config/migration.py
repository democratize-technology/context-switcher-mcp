...
class LegacyConfigAdapter:
    ...
    def _create_legacy_attributes(self):
        ...
        self.profiling = LegacyProfilingAdapter(
            self._unified_config.monitoring.profiling
        )
...
class LegacyModelConfigAdapter:
    ...
    def to_backend_config(self, backend) -> dict[str, Any]:
        """Legacy method compatibility"""
        return self._config.get_backend_config(
...
class LegacyRetryAdapter:
    ...
    def max_delay(self) -> float:
        ...
        return self._config.retry_delay_seconds * (
            self._config.retry_backoff_factor**self._config.max_retries
        )
...
class LegacyProfilingAdapter:
    ...
    def level(self) -> str:
        return (
            ...
            if hasattr(self._config.level, "value")
...
class LegacyServerAdapter:
    ...
    def log_level(self) -> str:
        return (
            ...
            if hasattr(self._config.log_level, "value")
            else self._config.log_level
        )


# Legacy configuration factory functions
def create_legacy_config_with_migration(
    config_file: str | Path | None = None, environment: str | None = None, **overrides
) -> LegacyConfigAdapter:
    ...
    try:
        ...
        unified_config = get_config(
            environment=environment, config_file=config_file, reload=True
        )

        # Apply any overrides by recreating with merged data
        if overrides:
            merged_config = ContextSwitcherConfig(config_file=config_file, **overrides)
