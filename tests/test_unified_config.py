...
from context_switcher_mcp.config import get_config, reload_config, ConfigurationError
from context_switcher_mcp.config.core import ContextSwitcherConfig, create_config
...
class TestDomainConfigurations:
    ...
    def test_security_configuration(self):
        ...
        assert security.enable_client_binding == True
        assert security.enable_rate_limiting == True
        assert security.enable_input_sanitization == True
        
        # Test configuration getters
        rate_limit = security.get_rate_limit_config()
        assert rate_limit["enabled"] == security.enable_rate_limiting
        assert rate_limit["requests_per_minute"] == security.rate_limit_requests_per_minute
    ...
    def test_server_configuration(self):
        ...
        dev_config = ContextSwitcherConfig(server={"log_level": "DEBUG", "enable_debug_mode": True})
    ...
    def test_monitoring_configuration(self):
        ...
        # Test default values
        assert monitoring.enable_monitoring == True
        assert monitoring.profiling.enabled == True
...
class TestEnvironmentConfigurations:
    ...
    def test_development_environment_config(self):
        ...
        assert config.server.log_level.value == "DEBUG"
        assert config.server.enable_debug_mode == True
        assert config.server.enable_hot_reload == True
        ...
        assert config.security.enable_client_binding == False
...
class TestLegacyCompatibility:
    ...
    def test_legacy_model_config_compatibility(self):
        ...
        with warnings.catch_warnings():
            ...
            assert legacy_config.model.bedrock_model_id == unified_config.models.bedrock_model_id
            assert legacy_config.model.ollama_host == str(unified_config.models.ollama_host)
    ...
    def test_legacy_server_config_compatibility(self):
        ...
        with warnings.catch_warnings():
            ...
            assert legacy_config.server.log_level == unified_config.server.log_level.value
    ...
    def test_legacy_session_config_compatibility(self):
        ...
        with warnings.catch_warnings():
            ...
            assert legacy_config.session.default_ttl_hours == unified_config.session.default_ttl_hours
            assert legacy_config.session.max_active_sessions == unified_config.session.max_active_sessions
            
            # Test legacy validation attributes (now in session domain)
            assert legacy_config.validation.max_session_id_length == unified_config.session.max_session_id_length
            assert legacy_config.validation.max_topic_length == unified_config.session.max_topic_length
    ...
    def test_legacy_profiling_config_compatibility(self):
        ...
        with warnings.catch_warnings():
            ...
            assert legacy_config.profiling.enabled == unified_config.monitoring.profiling.enabled
            assert legacy_config.profiling.level == unified_config.monitoring.profiling.level.value
            assert legacy_config.profiling.sampling_rate == unified_config.monitoring.profiling.sampling_rate
